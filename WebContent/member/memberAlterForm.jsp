...
$(document).ready(function(){
	...
	$('#btnConfirmNumber').click(function(){
		...
		$.ajax({
			...
			success:  function (check) {
				if(check){
					alert('인증번호가 일치합니다.');
					$('#btnAlter').removeAttr('disabled');
				}else{
					alert('인증번호가 일치하지않습니다.');
					$('#btnAlter').attr("disabled","disabled");
				}
			},
			failure: function () {
				alert('Fail');
	...
	$('#btnCancel').click(function(){
		location.href = "<%=application.getContextPath()%>/TscMainView.tsc";
	});
});
$(function() {
	$("#fm").validate({
...
<div id="page-wrapper">
    <div>
        ...
        <div class="col-xs-12 col-sm-7 col-md-7">
            <form id="fm"  name="alterForm" class="form-horizontal" action="<%=application.getContextPath()%>/MemberAlterAction.mem"role="form"method="post">
				...
				<div class="container">
				    <label for="inputid" class=" control-label" ></label>
				    <div class="container col-lg-3">
				      	<input type="text" class="form-control" id="name" name="name" placeholder="이름" tabindex="4" value="<%=bean.getName()%>">
				    </div>
				</div>
				<div class="container">
				    <label for="inputid" class=" control-label" ></label>
				    <div class="container col-lg-3">
				      	<input type="email" class="form-control" id="email" name="email" placeholder="이메일" tabindex="5" value="<%=bean.getEmail()%>">
				    </div>
				</div>
				<div class="container">
				    <label for="inputid" class=" control-label"></label>
					<div class="col-lg-2"> 
				   		<input type="number" class="form-control" id="phoneNumber" name="phoneNumber" size="11" placeholder="핸드폰번호" tabindex="6" value="<%=bean.getPhonenumber()%>">
					</div>
					<div class="col-lg-1" >
			    ...
			    <div class="container">
				    <div class="container col-lg-3">
					    <div>
					      	<input type="button" class="btn btn-info btn-lg btn-block" id="btnAlter" value="수정완료" disabled>
					      	<input type="button" class="btn btn-danger btn-lg btn-block" id="btnCancel" value="수정취소">
					    </div>
				    </div>
				</div>
			</form>
